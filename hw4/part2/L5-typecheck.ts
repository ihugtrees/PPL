// L5-typecheck
// ========================================================
import { equals, map, zipWith, chain, values, join } from 'ramda';
import {
	isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
	isPrimOp, isProcExp, isProgram, isStrExp, isVarRef, parseL5Exp, unparse,
	AppExp, BoolExp, DefineExp, Exp, IfExp, LetrecExp, LetExp, NumExp,
	Parsed, PrimOp, ProcExp, Program, StrExp, isLetValuesExp, LetValuesExp, ValuesBinding, CExp, VarDecl, isValuesBinding
} from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import {
	isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
	parseTE, unparseTExp,
	BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isEmptyTupleTExp, isTupleTExp, TupleTExp
} from "./TExp";
...
import { isTuple, Tuple } from './L5-value';
...
export const typeofLetValues = (exp: LetValuesExp, tenv: TEnv): Result<TExp> => {
	...
	const vars: VarDecl[][] = map((b) => b.vars, exp.bindings);
	const varTExps: TExp[][] = map((varlist) => map((v) => v.texp, varlist), vars);

	const allVars: string[] = chain((x) => map((v: VarDecl) => v.var, x), vars);
	const allVarTExp: TExp[] = chain((x) => x, varTExps);

	const vals: CExp[] = map((b) => b.val, exp.bindings)
	const valsTExps: Result<TExp[]> = mapResult((val) => typeofExp(val, tenv), vals)

	const tupelsTExps: Result<TExp[][]> = bind(valsTExps, (texps: TExp[]): Result<TExp[][]> =>
		mapResult((t: TExp) => isTupleTExp(t) ? isEmptyTupleTExp(t) ? makeOk([]) : makeOk(t.TEs) :
			makeFailure(`${unparseTExp(t)} is not a tuple`), texps))

	// const tupleSizeAndVarsSize = bind(tupelsTExps, (vals: TExp[][]) =>
	// 	bind(mapResult((vars: TExp[]) => {
	// 		const matchingVal = vals[varTExps.indexOf(vars)];
	// 		return vars.length === matchingVal.length ? makeOk(true) :
	// 			makeFailure(`size does not match ${vars} <-> ${vals}`);
	// 	}, varTExps), _ => makeOk(vals)));

	const constraints = (varsTExps: TExp[], tupelsTExp: TExp[]) =>
        zipWithResult((varTE, valTE) => varsTExps.length === tupelsTExp.length ? checkEqualType(varTE, valTE, exp) 
        : makeFailure(`sizes dont match ${varsTExps} != ${tupelsTExp}`),
			varsTExps, tupelsTExp);

	const mapConstraints = bind(tupelsTExps, (tupelsTExp: TExp[][]) =>
		mapResult((varsTExps: TExp[]) =>
			constraints(varsTExps, tupelsTExp[varTExps.indexOf(varsTExps)]), varTExps));

	return bind(mapConstraints, _ => typeofExps(exp.body, makeExtendTEnv(allVars, allVarTExp, tenv)));
}
...
// const balancedArrays2 = (vars: VarDecl[][], vals: CExp[]): Result<boolean> =>
// 	vars.length != vals.length ? makeFailure('let values had defrent sizes!!!') :
//     balancedArrays(vars, vals)

// const balancedArrays = (vars: VarDecl[][], vals: Tuple[]): Result<boolean> =>
// 	vars.length === 0 && vals.length === 0 ? makeOk(true) :
// 		vars[0].length !== vals[0].val.length ? makeFailure('let values size didnt match ${vars[0]} != ${vals[0]}') :
// 			balancedArrays(rest(vars), rest(vals))
...
const typeConsTExp = parseTE('(T1 * T2 -> Pair(T1, T2))');
const typeCarTExp = parseTE('(Pair(T1, T2) -> T1)');
const typeCdrTExp = parseTE('(Pair(T1, T2) -> T2)');
							...
							(p.op === 'string=?') ? parseTE('(T1 * T2 -> boolean)') :
								(p.op === 'display') ? parseTE('(T -> void)') :
									(p.op === 'newline') ? parseTE('(Empty -> void)') :
										(p.op === 'cons') ? typeConsTExp :
											(p.op === 'car') ? typeCarTExp :
												(p.op === 'cdr') ? typeCdrTExp :
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> => {
	if (isPrimOp(app.rator) && app.rator.op === "values")
		return typeofTuple(app.rands, tenv);
...
const typeofTuple = (rands: CExp[], tenv: TEnv): Result<TExp> =>
	bind(mapResult((rand: CExp) => bind(typeofExp(rand, tenv), unparseTExp), rands),
		(arr: string[]) => parseTE(`(${join(' * ', arr)})`))
