...
const evalLetValues = (exp: LetValueExp, env: Env): Result<Value> =>
    safe2((vars: VarDecl[][], vals: Tuple[]): Result<Value> =>
        ...
        (mapResult((b: ValuesBinding) => makeOk(b.vars), exp.bindings)
            , tuples(exp.bindings, env))

const tuples = (bnd: ValuesBinding[], env: Env): Result<Tuple[]> =>
    bind(mapResult((binding: ValuesBinding) =>
...
const combineVar = (vars: VarDecl[][]): string[] =>
    chain((x) => map((v: VarDecl) => v.var, x), vars);

// make one long array
const combineVal = (vals: Tuple[]): SExpValue[] => chain((x) => x.val, vals);
