...
% Purpose:defines the relation between a child name and its parent phone number

pick_me_up(Child_name,Phone) :- parents(Child_name,P1,_P2), parent_details(P1,Phone,true).
pick_me_up(Child_name,Phone) :- parents(Child_name,_P1,P2), parent_details(P2,Phone,true).
...
% Purpose:relationship between an activity name and list of all the childrens names that participate at this activity (without repetition).

activity_participants_list(Name,List) :- bagof(Child, participate(Child, Name), List).

% Signature: child_taken_day(Child_name,Day)/2
% Purpose: defines the relation between a child name and a day the child has activity on

child_taken_day(Child_name,Day) :- participate(Child_name,Activity), activity(Activity,Day).
...
% (the child is available at the day the activity takes place).

can_register(Child_name,Activity) :- activity(Activity,Day), bagof(TakenDay,child_taken_day(Child_name,TakenDay),TakenDays), not_member(Day,TakenDays).