...
(define sqrt2
  (lambda (x init epsilon)
    (car (find-first (sqrt-lzl x init) (lambda (lzl-guess) (good-enough? (car lzl-guess) x epsilon)))
         )
    )
  )
...
(define collect-all-values-2
  (lambda (lists key)
    (if (empty? lists)
        '()
        (if (equal? (get-value$ (car lists) key (lambda(x) x) (lambda() #f)) #f)
            (collect-all-values-1 (cdr lists) key)
            (cons (get-value$ (car lists) key (lambda(x) x) (lambda() #f)) (collect-all-values-1 (cdr lists) key))) 
        )
    )
  )
